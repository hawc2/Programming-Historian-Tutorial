...
print(paste(nrow(AllCommentsDF), "comments from", length(AllCommentsVideos), "unique videos identified using the", length(SearchTerms) ,"tags: ", paste(SearchTerms, collapse=", "),".  =)"))
```
...
### Here, eventually, I want to write code that adds 'short title' - the first 4? 5? words with >2 characters each
...
### Optional -- Save Raw Comment Data
```{r output_comments, eval=F, echo=F}
#output comments with their metadata
#write.csv(AllCommentsMetadata, paste("AllComments_", subject.s, ".csv", sep=""))
```



#PREPROCESS longer comments
...
for (com in 1:nrow(AllCommentsMetadata_aj)) {
  ...
  if(length (sto_text) >= 8 & length(unique(sto_text)) >=5 ) {   #@ comments with at least 8 words, 5 of which are unique
        #@ create list.head               
    list.head <- paste(AllCommentsMetadata_aj$videoTitle[com], "%_%", AllCommentsMetadata_aj$commentId[com]) # unique com names
    comment_tokens_l[[list.head]] <- sto_text
  }}

print(paste("keeping",length(comment_tokens_l), "out of", nrow(AllCommentsMetadata_aj),"comments"))


View(comment_tokens_l)
#View(AllComments_df)

comment_names <- names(comment_tokens_l)  ## CHANGED THE NAME OF THIS!!
beep("coin")
```
