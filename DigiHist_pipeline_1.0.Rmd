...
subject_s <- "COVID"  #@ what the project is about (this will appear in visualization titles and file names)
...
# Other relevant up-front parameters?
...
### chunk to read-in data
Do we want to keep this in?  Right now, it will be 'ignored' by the kint markdown report
```{r read_data, echo=F, results='hide'}
...
#test.csv.df <- read.csv(paste(data.dir, input.dir, files.v[1], sep="/"), quote = "", stringsAsFactors=FALSE)
#View(head(test.tab.df))
#nrow(test.csv.df)
```


# Code BEGINS here Proper
...
#safely
#get_comments_safely <- safely(get_all_comments, otherwise = NULL)
#AllCommentswErrors <- map(video_list, get_comments_safely)


### piping in data from the other script
AllComments_df <- AllCommentsMetadata_aj
