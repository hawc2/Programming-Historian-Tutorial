...
subject_s <- "Floyd Chauvin and BLM"
SearchTerms <- c("george floyd", "derek chauvin", "black lives matter")
...
# Save your data
write.csv(AllCommentsMetadata_aj, paste("Data/AllComments_", subject_s, ".csv", sep=""))
```
...
data <- read.csv(paste("Data/AllCommentsMetadata 6 9 20", ".csv", sep=""))  # -- using sample dataset - BLM
#data <- read.csv(paste("Data/AllCommentsMetadata 5 29 20", ".csv", sep=""))  # -- using sample dataset - COVID/Stay
...
for (com in 1:nrow(AllCommentsMetadata_aj)) {
  ...
  if(length (sto_text) >= 8 & length(unique(sto_text)) >=5) {   #@ comments with at least 8 words, 5 of which are unique
        #@ create list.head               
    list.head <- paste(AllCommentsMetadata_aj$videoTitle[com], "%_%", com,"%_%", AllCommentsMetadata_aj$commentId[com], sep="")
...
nrow((comment_identifiers))



### CREATE DOC DATA DATAFRAME
length(alpha)
wf_docdata.df <- data.frame(wfdocs_v, theta, alpha)
View(wf_docdata.df)
