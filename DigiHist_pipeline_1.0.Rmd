...
#data <- read.csv(paste("Data/AllCommentsMetadata 6 9 20", ".csv", sep=""))  # -- using sample dataset
data <- read.csv(paste("Data/AllCommentsMetadata 5 29 20", ".csv", sep=""))  # -- using sample dataset
...
for (com in 1:nrow(AllCommentsMetadata_aj)) {
  ...
  if(length (sto_text) >= 50 & length(unique(sto_text)) >=10 ) {   #@ comments with at least 8 words, 5 of which are unique
        #@ create list.head               
...
#wfa1$docs <- names(comment_list)  #@ I ACTUALLY need the 'names' of the final set of comments, after bad comments are removed
