...
if(any(duplicated(AllCommentsDF$id))==T){
  ...
  AllCommentsDF <- AllCommentsDF %>% distinct(id, .keep_all = FALSE)
}
nrow(AllCommentsDF)
...
# Save your data
write.csv(AllCommentsMetadata, paste("Data/AllComments_", subject_s, today("EST"),".csv", sep=""))
```
...
weird_words <- wf_worddata.df$wfwords_v[which((wf_worddata.df$psi < -9))]  # views extremely rare words that were kept
#weird_words
...
#subject_s <- "COVID_and_Opening_America_10 word min comments"  
...
channel_T_A_plot <- ggplot(data = wf_docdata.df, mapping = aes(x =theta, y = alpha, label = comment_number, color=video_channel)) + 
  geom_text(size = 1.2) + 
  labs(x = "Comment polarity: an optimized value (theta)", y = "Comment length: a fixed effect (alpha)") +
  guides(size = "none", color = guide_legend("")) + theme(legend.position="bottom") + 
  theme(legend.text = element_text(size=5)) +
